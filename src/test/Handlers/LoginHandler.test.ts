...
describe('LoginHandler test suite', () => {
    ...
    test('not handled http method', async () => {
        requestMock.method = 'anyrandomnethod';
        await loginHandler.handleRequest();
        expect(responseMock.writeHead).not.toHaveBeenCalled();
    })

    test('post valid request auth with valid login', async () => {
    ...
    test('post valid request auth with invalid login', async () => {
        getRequestBodyMock.mockReturnValueOnce({
            username: "someUser",
            password: "password"
        })
    ...
    test.only('post valid request auth with unexpected error', async () => {
